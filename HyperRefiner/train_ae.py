...
def train_ae(epoch):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        MS_image = Variable(MS_image.float().cuda())  # 8*145*30*30  BChw
        PAN_image = Variable(PAN_image.float().cuda())  # 8*120*120  BHW
        en, de = model(MS_image)
        # de = model(MS_image)
        outputs = de
        reference = MS_image
        # en = F.interpolate(en,scale_factor=(4,4),mode='bilinear')
        # outputs = out["pred"]
        # aux_out = out["aux_pred"]
        # tp_loss = out["tp_loss"][0]
        ######### Computing loss #########
...
def test_ae(epoch):
    ...
    ms = torch.unsqueeze(MS_image.view(-1, MS_image.shape[-2], MS_image.shape[-1]), 1)
    pred = torch.unsqueeze(outputs.view(-1, outputs.shape[-2], outputs.shape[-1]), 1)
    # en = torch.unsqueeze(en.view(-1, en.shape[-2], en.shape[-1]), 1)
    ref = torch.unsqueeze(reference.view(-1, reference.shape[-2], reference.shape[-1]), 1)
    imgs = torch.zeros(3 * pred.shape[0], pred.shape[1], pred.shape[2], pred.shape[3])
    for i in range(pred.shape[0]):
        imgs[3 * i] = ref[i]
        imgs[3 * i + 1] = torch.abs(ref[i] - pred[i]) / torch.max(torch.abs(ref[i] - pred[i]))
        # imgs[3 * i + 1] = en[i]
        imgs[3 * i + 2] = pred[i]  # imgs[3 * i + 3] = ref[i]  # imgs[3 * i + 4] = torch.abs(ref[i] - ms[i]) / torch.max(torch.abs(ref[i] - ms[i]))
...
def test_vae(epoch):
    ...
    # Writing test results to tensorboard
    writer.add_scalar('Loss/test', test_loss, epoch)
    writer.add_scalar('Test_Metrics/recl', recl, epoch)
    writer.add_scalar('Test_Metrics/KLD', kld, epoch)
    writer.add_scalar('Test_Metrics/CC', cc, epoch)
    writer.add_scalar('Test_Metrics/SAM', sam, epoch)
    writer.add_scalar('Test_Metrics/RMSE', rmse, epoch)
    writer.add_scalar('Test_Metrics/ERGAS', ergas, epoch)
...
def pre_train_ae_val(epoch):
    ...
    if config["trainer"]["is_small_patch_train"]:
        ...
        outputs = outputs.contiguous().view(
                config["val_batch_size"], config[config["train_dataset"]]["spectral_bands"],
                config[config["train_dataset"]]["HR_size"], config[config["train_dataset"]]["HR_size"]
                )
        reference = reference.view(unfold_shape).permute(0, 1, 4, 2, 5, 3, 6).contiguous()
        reference = reference.contiguous().view(
                config["val_batch_size"], config[config["train_dataset"]]["spectral_bands"],
                config[config["train_dataset"]]["HR_size"], config[config["train_dataset"]]["HR_size"]
                )
        MS_image = MS_image.view(unfold_shape).permute(0, 1, 4, 2, 5, 3, 6).contiguous()
        MS_image = MS_image.contiguous().view(
                config["val_batch_size"], config[config["train_dataset"]]["spectral_bands"],
                config[config["train_dataset"]]["HR_size"], config[config["train_dataset"]]["HR_size"]
                )
...
if __name__ == "__main__":
    __dataset__ = {"pavia_dataset": pavia_dataset, "botswana_dataset": botswana_dataset, "chikusei_dataset": chikusei_dataset,
                   "botswana4_dataset": botswana4_dataset, "beijing_dataset": beijing_dataset, "beijing_train_dataset": beijing_train_dataset}

    # Parse the arguments
    parser = argparse.ArgumentParser(description='PyTorch Training')
    parser.add_argument('-c', '--config', default='configs/config_ae.json', type=str, help='Path to the config file')
    ...
    PATH = "./" + config["experim_name"] + "/" + config["train_dataset"] + "/" + "N_" + str(config["N_modules"])
    ensure_dir(PATH + "/")
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if epoch % config["trainer"]["test_freq"] == 0:
            ...
            image_dict, pred_dic, metrics = test_ae(epoch)
            #pred_dic, metrics = pre_train_ae_val(epoch)
