...
def train_ae(epoch):
    ...
    for i, data in enumerate(train_loader, 0):
        # Reading data.
        if config["is_DHP_MS"]:
            _, MS_image, PAN_image, reference, MS_dhp = data
        else:
        ...
        if config["sr_loss"]:
            if config[config["train_dataset"]]["Normalized_L1"]:
                ...
                rc_loss = criterion(outputs / max_ref, to_variable(reference) / max_ref)
                sr_loss = criterion(coarse_hsi / max_ref, to_variable(reference) / max_ref)
            else:
                rc_loss = criterion(outputs, to_variable(reference))
                sr_loss = criterion(coarse_hsi, to_variable(reference))

            # Spatial loss
            if config[config["train_dataset"]]["Spatial_Loss"]:
                rc_loss += config[config["train_dataset"]]["Spatial_Loss_F"] * Spatial_loss(to_variable(reference), outputs)
                sr_loss += config[config["train_dataset"]]["Spatial_Loss_F"] * Spatial_loss(to_variable(reference), coarse_hsi)
        ...
        # aux loss, 根据训练情况动态调整权重
        if config["dynamic"]:
            if sr_loss > rc_loss:
                loss = sr_loss * (rc_loss / (rc_loss + sr_loss)) + rc_loss * (sr_loss / (rc_loss + sr_loss))
            else:
                loss = rc_loss * (rc_loss / (rc_loss + sr_loss)) + sr_loss * (sr_loss / (rc_loss + sr_loss))
        else:  # 固定权重
            loss = config['hsi_loss_F'] * rc_loss + config['sr_loss_F'] * sr_loss
...
def test_ae(epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader, 0):
            ...
            if config["is_DHP_MS"]:
                ...
                out = model(PAN_image, MS_image, None)
            # Taking model output
            out_aux = out['coarse_hsi']
            outputs = out['pred']
            de_lr_hsi = out['de_lr_hsi']

            # Computing validation loss
            loss = criterion(outputs, reference)
            aux_loss = criterion(out_aux, reference)

            test_loss += loss.item()
            aux_test_loss += aux_loss.item()
            ...
            out_aux[out_aux < 0] = 0.0
            out_aux[out_aux > 1.0] = 1.0
            out_aux = torch.round(out_aux * config[config["train_dataset"]]["max_value"])
            ...
            cc_aux += cross_correlation(out_aux, reference)
            # SAM
            sam += SAM(outputs, reference)
            sam_aux += SAM(out_aux, reference)
            # RMSE
            rmse += RMSE(outputs / torch.max(reference), reference / torch.max(reference))
            rmse_aux += RMSE(out_aux / torch.max(reference), reference / torch.max(reference))
            # ERGAS
            ...
            ergas_aux += ERGAS(out_aux, reference, beta)
            # PSNR
            psnr += PSNR(outputs, reference)
            psnr_aux += PSNR(out_aux, reference)
    ...
    cc_aux /= len(test_loader)
    sam_aux /= len(test_loader)
    rmse_aux /= len(test_loader)
    ergas_aux /= len(test_loader)
    psnr_aux /= len(test_loader)
    ...
    writer.add_scalar('Loss/test_aux', aux_test_loss, epoch)
    writer.add_scalar('Test_Metrics/CC_aux', cc_aux, epoch)
    writer.add_scalar('Test_Metrics/SAM_aux', sam_aux, epoch)
    writer.add_scalar('Test_Metrics/RMSE_aux', rmse_aux, epoch)
    writer.add_scalar('Test_Metrics/ERGAS_aux', ergas_aux, epoch)
    writer.add_scalar('Test_Metrics/PSNR_aux', psnr_aux, epoch)

    # Images to tensorboard
    # Normalizing the images
    outputs = outputs / torch.max(reference)
    out_aux = out_aux / torch.max(reference)
    reference = reference / torch.max(reference)

    pred_aux = torch.unsqueeze(out_aux.view(-1, out_aux.shape[-2], out_aux.shape[-1]), 1)
    ...
    for i in range(pred.shape[0]):
        imgs[5 * i] = ref[i]
        imgs[5 * i + 1] = torch.abs(ref[i] - pred[i]) / torch.max(torch.abs(ref[i] - pred[i]))
        imgs[5 * i + 2] = torch.abs(ref[i] - pred_aux[i]) / torch.max(torch.abs(ref[i] - pred_aux[i]))
        imgs[5 * i + 3] = pred[i]
        imgs[5 * i + 4] = pred_aux[i]
    ...
    metrics_aux = {"aux_loss": float(aux_test_loss), "cc_aux": float(cc_aux), "sam_aux": float(sam_aux), "rmse_aux": float(rmse_aux),
                   "ergas_aux": float(ergas_aux), "psnr_aux": float(psnr_aux)}
    metrics = {"pred": metrics_pred, "aux": metrics_aux}
...
if __name__ == "__main__":
    __dataset__ = {"pavia_dataset": pavia_dataset, "botswana_dataset": botswana_dataset, "chikusei_dataset": chikusei_dataset,
                   ...
                   ,"beijing_train_dataset": beijing_train_dataset, "road_train_dataset": road_train_dataset}
    device_ids = [0,]
    # Parse the arguments
    parser = argparse.ArgumentParser(description='PyTorch Training')
    parser.add_argument('-c', '--config', default='configs/config_hyae1.json', type=str, help='Path to the config file')
    parser.add_argument('-r', '--resume', default=None, type=str, help='Path to the ae.pth model checkpoint to resume training')
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if epoch % config["trainer"]["test_freq"] == 0:
            ...
            image_dict, pred_dic, metric = test_ae(epoch)
